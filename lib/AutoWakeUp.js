	...
	// A mettre un 1 quand test finis
		...
		return new Promise(function(destination, reject) {
			request('http://photon.komoot.de/api/?q='+loca_event, function(err, result, body) {
				...
				destination = `${loca_event.features[0].geometry.coordinates[0]};${loca_event.features[0].geometry.coordinates[1]}`;
				console.log(from)
			...
			console.log(url)
				...
				request('https://'+shared.api+url+from+'&to='+destination+'&first_section_mode[]='+options.mode+'&min_nb_journeys=10&', function(err, response, body) {
