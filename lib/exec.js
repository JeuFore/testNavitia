...
module.exports = function get(options){
	...
	return new Promise(function(resolve, reject) {
		...
		if (options.depart) {
			from = options.depart
			x = from.replace(/\s/g, "%20").replace(/,/g,"%2C").replace(/é/g,"%C3%A9");

			request('http://photon.komoot.de/api/?q='+x, function(err, result, body) {
				x = JSON.parse(body);

				console.log(x)

				var from = (`${x.features[0].geometry.coordinates[0]};${x.features[0].geometry.coordinates[1]}`)
		...
		if (options.destination) {
			...
			request('http://photon.komoot.de/api/?q='+options.destination, function(err, result, body) {
				...
				var to = (`${to.features[0].geometry.coordinates[0]};${to.features[0].geometry.coordinates[1]}`)
	...
	var urlSansToken =  'https://TOKEN@api.navitia.io/v1/journeys?from='+from+'&to='+destination+'&first_section_mode[]='+options.mode
	var url = 'https://'+shared.api+'@api.navitia.io/v1/journeys?from='+from+'&to='+destination+'&first_section_mode[]='+options.mode

	if (options.coverage) {

		url = 'https://'+shared.api+'@api.navitia.io/v1/coverage/'+options.coverage+'/journeys?from='+from+'&to='+destination+'&first_section_mode[]='+options.mode
		urlSansToken = 'https://TOKEN@api.navitia.io/v1/coverage/'+options.coverage+'/journeys?from='+from+'&to='+destination+'&first_section_mode[]='+options.mode
	}
